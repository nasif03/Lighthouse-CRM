...
function CallButton({ participantName, phoneNumber }: { participantName: string; phoneNumber?: string }) {
  const { token } = useAuthStore();
  const { makeCall, isLoading, error, clearError } = useTwilioStore();
  ...
  const handleCall = async () => {
    ...
    // Always use the hardcoded allowed number
    const allowedNumber = '+8801957128594';
    
    try {
      clearError();
      setCallStatus('Calling...');
      const result = await makeCall(token, allowedNumber, `Hello ${participantName}, this is a call from Lighthouse CRM.`);
      setCallStatus(`Call initiated: ${result.status}`);
      ...
      const errorMessage = err.message || 'Failed to make call';
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      {callStatus && (
        <span className="text-xs text-gray-600 max-w-[100px] text-right">{callStatus}</span>
      )}
    </div>
  );
}
